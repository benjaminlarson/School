import sys, math, glob, os, string
...
def gen_webpage(num_cols, size, src, name="index"):
  ...
  for file in allfiles: #glob.glob( "*.png" ):
    ...
    #thumb_filename = string.replace( thumb_filename, "tif", "png")
    print "writing " + thumb_filename
...
if __name__ == "__main__":
  if len(sys.argv) != 3:
    print "usage " + sys.argv[0] + " <num cols> <thumb size>"
    exit()

  num_cols = int( sys.argv[1] )
  size = sys.argv[2]
  gen_webpage(num_cols, size, '.')
