...
def plot_embedding(X, y, C, title=None):
    ...
    for i in range(X.shape[0]):
        ...
        # Plot class number
        plt.text(X[i, 0]+0.01, X[i, 1], C[i*3], color=plt.cm.Set1(y[i]/10.) )
        # Plot bird name
        # if ( i  % 3 == 1):
        #   name = C[i] 

        plt.text(X[i, 0], X[i, 1], name, color=plt.cm.Set1(y[i]/10.) )
        # print  "Class ", C[ i*3 ] 
    ...
    if hasattr(offsetbox, 'AnnotationBbox'):
        ...
        for i in range( len( bird_images_show ) ) : #digits.data.shape[0]):
            ...
            if np.min(dist) < 4e-2:  #4e-3:
                # don't show points that are too close
                continue
            shown_images = np.r_[shown_images, [X[i]]]
            imagebox = offsetbox.AnnotationBbox(
                offsetbox.OffsetImage(bird_images_show[i].convert() ), #cmap=plt.cm.gray),
                X[i],
                frameon=False,
                pad = 0.2)
...
# print y_pred
# print birdClass[0]
plot_embedding(X_pca, y_pred, birdClass[0:],
