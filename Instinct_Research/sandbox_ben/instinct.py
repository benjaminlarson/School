...
def plot_embedding(plt, X, y, DOI, FV, title=None):
	...
	for i in range(X.shape[0]):
    		plt.text(X[i, 0], X[i, 1], "x", color=plt.cm.Set1(y[i]/15.) ) 
...
def ComputeSVM(FV, DOI):
	...
	k_means = KMeans(n_clusters=3, init = 'random', n_init = 15, random_state=0) #fixing the RNG is kmeans
	k_means.fit(dTrain)
	y_pred = k_means.predict(dTest) 

	crt = 0
	fl = 0
	##accuracy for histograms. Add number of classes to correct count. 
