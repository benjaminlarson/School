...
badgesTrainF = hm5.readFeatures('badges/badges-train-features.txt') 
badgesTestF = hm5.readFeatures('badges/badges-test-features.txt') 
...
for x in badgesTrainF:
	...
	for y in x: 
		...
		if len(L) > 1: 
			row[ L[0] ] = L[1]
			attributes_array.append(L[0])
	data_input_array.append(row) 	
...
def print_tree(tree, string):
	...
	if type(tree) == dict:
		...
		print "%s Keys/NodeToSplitOn: %s" % (string, tree.keys()[0])
		for item in tree.values()[0].keys():
			print "%s \tValueofKey: %s" % (string, item)
			print_tree(tree.values()[0][item], string + "\t")
	else:
		print "%s\t->\tValue: %s" % (string, tree)
