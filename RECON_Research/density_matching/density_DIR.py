...
def createData(x,seg,HighT=None,LowT=None, La_Water = 0.2, power = None, fit_fn = None):
	...
	if HighT or LowT: 
		...
		newseg.setGrid(grid)
		WriteMHA(newseg,'/home/sci/benl/Data_DIR/hard_segs/01.mha')
...
# for i,j in zip(im, HighThreshold):
# 	# images (path_mha) and where to store pkl,pngs (path)
# 	# process('/scratch/Aa_HumanSub/'+str(i)+'/','/scratch/Aa_HumanSub/Human_pkl/'+str(i), j)
# 	# missingMass('/scratch/Aa_HumanSub/Human_pkl/'+str(i))
# 	fyr.Create_Pkl('/home/sci/benl/Aa_HumanSub/'+str(i)+'/',
# 		'/scratch/Aa_HumanSub/Human_pkl/'+str(i),
# 		j,
# 		La_Water = 0.17,
# 		fit_fn = fit_fn2)
# 	fyr.Histogram_Pkl('/home/sci/benl/Aa_HumanSub/'+str(i)+'/','/scratch/Aa_HumanSub/Human_pkl/'+str(i),j)
...
for num in ['06']: 
	...
	'''No hard threshold '''
	for i,p in zip(matches_i,matches_seg):
		# images (path_mha) and where to store pkl,pngs (path)
		# print zip(matches_i,matches_seg)
		# missingMass('/scratch/Aa_HumanSub/Human_pkl/'+str(i))
		print i,p 
	Create_Pkl(matches_i,matches_seg,
		'/home/sci/benl/Data_DIR/Density_500/'+num+'/',
		HighT = 800,
		LowT = None,
		power = 1/0.57
		)
	# Histogram_Pkl(i,
		# '/scratch/Data/Results/'+str(p),
		# None,
		# None,
		# La_Water = 0.17)
